...
def get_bottle_plan():
    ...
    with db.engine.begin() as connection:    
        total_red_ml = connection.execute(sqlalchemy.text("SUM(change) FROM ml_ledger WHERE color = red")).scalar_one()
        total_green_ml = connection.execute(sqlalchemy.text("SUM(change) FROM ml_ledger WHERE color = green")).scalar_one()
        total_blue_ml = connection.execute(sqlalchemy.text("SUM(change) FROM ml_ledger WHERE color = blue")).scalar_one()
        total_dark_ml = connection.execute(sqlalchemy.text("SUM(change) FROM ml_ledger WHERE color = dark")).scalar_one()
